...
function startPreloader() {
    ...
    preloaderArray.push("img/earth-glow.png");
...
function checkPreloader() {
    ...
    TweenMax.to("#preloaderBarInner", 1, {
        scaleX: tempPercentage, autoAlpha: 1, transformOrigin: 'center center', ease: Expo.easeOut, onComplete: function () {
...
function createGlobe(callbackFn) {
    ...
    // EARTH GLOW
    globeGlowSize = 200;
    globeGlowTexture = new THREE.TextureLoader().load("img/earth-glow.png");
...
function showGlobe() {
    ...
    TweenMax.fromTo(starsZoomObject.position, 6, { z: 0 }, {
        z: 325, ease: Circ.easeInOut,
        onComplete: function () {
    ...
    if (intersects.length > 0) {
        ...
        if (!isParticleHit || tempCurrentLocationTitle != currentLocationTitle) {
            ...
            if (!TweenMax.isTweening(tempHoverTarget.scale)) {
                ...
                TweenMax.fromTo(tempHoverTarget.material, 1.5, { opacity: 1 }, {
                    opacity: 0,
                    onStart: function () {
...
function generateExplosion() {
    ...
    explosionAnimation.fromTo(ringExplosionMesh.material, 0.25, { opacity: 0 }, {
        opacity: 1, ease: Linear.easeNone,
        onStart: function () {
            ringExplosionMesh.visible = true;
        }
    }, 0);
    explosionAnimation.fromTo(ringExplosionMesh.material, 0.75, { opacity: 1 }, {
        opacity: 0, immediateRender: false, ease: Linear.easeNone,
        onComplete: function () {
            ringExplosionMesh.visible = false;
        }
    }, 0.25);
    ...
    */
    ...
    glitcherAnimation.fromTo($('#glitcher'), 0.25, {
        x: generateRandomNumber(-15, 15), y: generateRandomNumber(-15, 15)
            ...
            x: 0, y: 0, ease: RoughEase.ease.config({ strength: 5, points: 50 })
        }, 0);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "25%", height: 15, autoAlpha: 1, ease: Linear.easeNone
    }, 0);  //, autoAlpha: (generateRandomNumber(25,100)/100)
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "25%", height: 8, ease: Linear.easeNone
    }, .05);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "10%", height: 5, ease: Linear.easeNone
    }, .1);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "15%", height: 5, ease: Linear.easeNone
    }, .15);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "35%", height: 1, ease: Linear.easeNone
    }, .2);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
        width: "10%", height: 8, ease: Linear.easeNone
    }, .25);
    glitcherAnimation.set($('#glitcher .minibar'), {
        left: generateRandomNumber(0, 90) + "%",
        top: generateRandomNumber(0, 90) + "%",
